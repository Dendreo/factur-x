...
class Facturx
{
    ...
    const FACTURX_PROFIL_TO_XSD = array(
        ...
        'basicwl' => 'BasicWL/FACTUR-X_BASIC-WL.xsd',
    ...
    {
        define('FPDF_FONTPATH', resource_path('pdf_fonts/'));
    ...
    private function checkXmlWithXsd(string $xmlContent): bool
    {
        ...
        if (!array_key_exists($profile, self::FACTURX_PROFIL_TO_XSD)) {
    ...
    private function getXmlProfile(\DOMDocument $xml): string
    {
        ...
        if (!array_key_exists(strtolower($profile), self::FACTURX_PROFIL_TO_XSD)) {
    ...
    private function updatePdfMetadata(FdpiFacturx $pdfWriter, \DOMDocument $document): FdpiFacturx
    {
        ...
        $descriptionFx->children('fx', true)->ConformanceLevel =
            strtoupper(self::FACTURX_PROFIL_TO_XMP[$this->profile]);
